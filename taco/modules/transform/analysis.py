...
# # Define AFA module for injecting text embeddings into image encoder
# class AttentionFeatureAggregation(nn.Module):
#     def __init__(self, dim, text_feature_dim, num_heads=8):
#         super().__init__()
#         self.num_heads = num_heads
#         self.query_proj = nn.Linear(dim, dim)
...
#     def forward(self, image_features: torch.Tensor, text_features: torch.Tensor):
#         """
#         :param image_features: [batch_size, channels, height, width] - Image features from the encoder
#         :param text_features: [batch_size, text_feature_dim] - Textual embeddings
#         :return: Updated image features with injected text information
#         """
#         # Reshape image features for attention: [batch_size, seq_len, channels]
#         b, c, h, w = image_features.size()
#         image_features = image_features.contiguous().view(b, c, h * w).permute(0, 2, 1)  # [batch_size, seq_len, channels]
...
# # Updated AnalysisTransformEX with AFA-based text injection
# class AnalysisTransformEX(nn.Module):
#     def __init__(self, N, M, text_embedding_dim, act=nn.ReLU):
#         super().__init__()
#         self.analysis_transform = nn.Sequential(
...
#             ResidualBottleneck(N, act=act),
#             ResidualBottleneck(N, act=act),
#             AttentionFeatureAggregation(N, text_embedding_dim),  # Replace Injector with AFA
#             AttentionBlock(N),
#             conv(N, N),
#             ResidualBottleneck(N, act=act),
#             ResidualBottleneck(N, act=act),
#             ResidualBottleneck(N, act=act),
#             AttentionFeatureAggregation(N, text_embedding_dim),  # Inject text embeddings again
#             conv(N, M),
#             AttentionBlock(M)
#         )
...
#         for layer in self.analysis_transform:
#             if isinstance(layer, AttentionFeatureAggregation):
#                 x = layer(x, text_embeddings)
#             else:
#                 x = layer(x)
#         return x
