...
DATASET_NAME_MAPPING = {
    "pickapic_clic2021": ("jpg_0", "jpg_1", "label_0", "caption"),
}
...
def main():
    ...
    ref_hyper_enc = HyperEncoder.from_pretrained(args.pretrained_model_name_or_path, subfolder="hyper_enc")
    ...
    if args.train_method == 'dpo': ref_hyper_enc.requires_grad_(False)
    unet.train()
    hyper_enc.train()
    ...
    # load custom dataset for ddpo 
    
    # output_dir = '/scratch/09004/sravana/perco_ddpo/parquet'
    output_dir = '/raid/sa53869/datasets/OpenImagesV6_sample_v0/synth/parquet'
    parquet_files = [os.path.join(output_dir, f) for f in os.listdir(output_dir) if f.startswith('dataset')]
    ...
    with accelerator.main_process_first():
        ...
        train_dataset_ref = OpenImagesV6(root=cfg_perco.data_dir,
                                     split='train',
    ...
    train_dataloader_ref = torch.utils.data.DataLoader(
        train_dataset_ref,
        shuffle=True,
    ...
    unet, hyper_enc, ref_unet, ref_hyper_enc, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, hyper_enc, ref_unet, ref_hyper_enc, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if len(batch['input_ids']) < args.train_batch_size:
                print(f"Skipping last batch of size {len(batch['input_ids'])} (smaller than batch size {args.train_batch_size})")
            ...
            with accelerator.accumulate(unet, hyper_enc):
                ...
                if args.train_method == 'dpo':
                    ...
                    images_denorm = feed_pixel_values * 127.5 + 127.5
                ...
                if args.train_method == 'sft': # SFT, casting for F.mse_loss
                    ...
                    with torch.no_grad(): # Get the reference policy (unet) prediction
                        ...
                        ref_hyper_latent = ref_hyper_enc(latents)
                        ref_z, ref_z_hat, ref_z_hat_indices, ref_commit_loss = ref_hyper_latent.z, ref_hyper_latent.z_hat, ref_hyper_latent.indices, ref_hyper_latent.commit_loss
                        
                        ref_pred = ref_unet(noisy_latents, timesteps, encoder_hidden_states, ref_z_hat, return_dict=False)[0].detach()
